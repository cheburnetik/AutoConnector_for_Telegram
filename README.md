...
Десктоп-версия — тот же движок и тот же интерфейс, что и на телефоне (общий
код на Kotlin Multiplatform + Compose). Готовый `.exe` — на странице релизов:
...
- **Закрытие окна сворачивает приложение в системный трей** (значок у часов),
  релей продолжает работать в фоне. Клик по значку — развернуть обратно;
  правый клик → меню «Открыть» / «Выход».
- В Telegram Desktop пропишите SOCKS5-прокси `127.0.0.1` порт `55001`
  (и `55002`) — как и на Android.

Данные (база прокси, настройки) хранятся в `%APPDATA%\AutoConnector`.
