...
/**
 * Process-wide singleton that records the SAMPLED running-probe-thread count per
 * second and per minute, feeding the Scan tab's "threads" graph.
 *
 ...
 */
public final class ThreadsBuffer {

    public static final int N = 60;
    public static final ThreadsBuffer INSTANCE = new ThreadsBuffer();
    ...
    /**
     * Records the current running-thread count. The latest sample of each
     * second / minute wins (overwrite), so the graph tracks the gauge value.
     */
    public synchronized void tick(long count) {
        ...
        secBuckets[(int) (((curSec % N) + N) % N)] = count;
        minBuckets[(int) (((curMin % N) + N) % N)] = count;
    }
