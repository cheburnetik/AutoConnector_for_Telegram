...
object Ru : Strings {
    ...
    override val histWho = "Кто"
    override val histWhen = "Когда"
    override val histReq = "Запрос"
    override val histSess = "Сессия"
    override val histScan = "скан"
    override val testNow = "Тестировать сейчас"
    override val testShort = "Тест"
    override val testResult = "Результат теста"
    override val testStop = "Стоп"
    override val testingNow = "идёт тест…"
    override val prewarmTitle = "Прогрев сокетов (эксперимент)"
    override val prewarmHelp = "Держать заранее открытыми несколько сокетов к прокси, чтобы новое " +
        "подключение Telegram пропускало коннект/хендшейк. Экспериментально: фон постоянно " +
        "переподключается → расход трафика и немного CPU. Фейковый трафик не шлётся (он сломал бы " +
        "реальную сессию) — сокеты просто ротируются. Полезнее всего с FakeTLS-прокси."
    override val prewarmEnable = "Включить прогрев"
    override val prewarmModeDeferred = "Отложенный (только TLS)"
    override val prewarmModeDeferredSub = "Держим TCP + FakeTLS; obfuscated2-init досылаем при передаче. DC не коммитится — реалистичнее всего."
    override val prewarmModeFull = "Полный хендшейк"
    override val prewarmModeFullSub = "Держим полностью подключённые сокеты по разным DC; переиспользуем только при совпадении DC/tag. Живут меньше."
    override val prewarmPoolLabel = "Сокетов про запас"
    override val prewarmHoldLabel = "Держать, с"
    override val prewarmNote = "Только ротация (без keepalive на уровне приложения). Сокет живёт секунды…~минуту, зависит от прокси/DC."
    override val prewarmStatus = "Прогрев"
    override fun prewarmStatusVal(ready: Int, holdSecs: Int) = "$ready готовы · держу ${holdSecs}с"
    override val prewarmStar = "Жирный оранжевый = сокет отдан тёплым из пула прогрева (без коннекта/хендшейка)"
    override fun prewarmTableTitle(holdSecs: Int) = "Прогрев сокетов (держу ${holdSecs}с)"
    override val prewarmTableHelp = "«Прогрев сокетов» заранее открывает несколько сокетов к прокси, чтобы новое " +
        "подключение Telegram пропускало коннект/хендшейк. В этой таблице показаны прогреваемые хосты: сколько секунд " +
        "живёт сокет, использует ли его Telegram, и трафик. Включить/выключить и настроить (режим, число сокетов, " +
        "время удержания) можно в «Ещё → Настройки → „Прогрев сокетов (эксперимент)“»."
    override val prewarmNoneWarming = "пока нет прогреваемых сокетов"
    override val prewarmColAge = "живёт"
    override val prewarmColUse = "в TG?"
    override val prewarmInUse = "в TG"
    override val prewarmNew = "новый"
    override val lanShareTitle = "Раздача по локальной сети (веб-портал)"
    override val lanShareDesc = "Разрешить другим устройствам в этом Wi-Fi использовать этот АвтоКоннектор как прокси; браузер по адресу ниже получит страницу с лучшими прокси."
    override val lanShareUrlsLabel = "Соседи по сети подключаются:"
    override val lanShareNoIp = "нет адреса в локальной сети — подключитесь к Wi-Fi"
    override val lanFirewallTitle = "Разрешить в локальной сети"
    override val lanFirewallBody = "При включении порты релея откроются в локальную сеть. Сейчас файрвол Windows (или другой) может спросить, разрешить ли AutoConnector — выберите «Разрешить»/«Да». Если запретить, трафик соседей к АвтоКоннектору будет заблокирован, и страница/прокси будут недоступны."
    override val lanFirewallConfirm = "Включить"
    override val lanInfoTitle = "Зачем это?"
    override val lanInfoBody = "Запустите АвтоКоннектор на ОДНОМ компьютере или телефоне в вашем Wi-Fi — и все остальные устройства в той же сети, включая iPhone (который приложение напрямую не поддерживает), смогут просто открыть адрес в браузере и пользоваться: страница с лучшими прокси для добавления в их Telegram, либо сам этот девайс как SOCKS-прокси. Одно устройство находит и держит прокси, остальные пользуются им по локальной сети."
    override val volTriggerTitle = "Триггер кнопками громкости"
    override val volTriggerSub = "Переключение прокси быстрым паттерном клавиш громкости"
    override val volEnableLabel = "Следить за кнопками громкости"
    override val volHelpTitle = "Что это?"
    override val volHelpBody = "На Android нет глобальных клавиатурных хоткеев, поэтому используются кнопки ГРОМКОСТИ. Когда включено, АвтоКоннектор в фоне следит за быстрым паттерном нажатий громкость-вверх/вниз (например вверх-вверх-вниз-вниз). Распознав его, он открывает tg://-ссылку случайного хорошего живого прокси — Telegram её перехватывает и переключается. Быстрый незаметный способ ротации прокси, не открывая приложение. Громкость работает как обычно (нажатия не перехватываются). Нужен доступ к спец.возможностям (чтобы читать клавиши в фоне и открывать ссылку); ничего не запрашивается, пока не включите галочку. Ниже задайте максимум времени между нажатиями; распознаваемые паттерны перечислены внизу."
    override val volGrantTitle = "Включите спец.возможности (важно)"
    override val volGrantBody = "Android (особенно 13+) блокирует спец.возможности для приложений, установленных НЕ из Google Play — поэтому AutoConnector серый и пишет «Доступ для приложения запрещён».\n\nКак разблокировать:\n1. Откройте «О приложении» (кнопка ниже, или: Настройки → Приложения → AutoConnector for Telegram).\n2. Нажмите ⋮ (три точки вверху справа) → «Разрешить ограниченные настройки» → подтвердите.\n3. Вернитесь: Настройки → Спец.возможности → AutoConnector for Telegram → включите.\n\nЕсли пункта «Разрешить ограниченные настройки» нет — сначала один раз попробуйте включить переключатель в спец.возможностях (появится сообщение о запрете), затем шаг 2 станет доступен.\n\nНа Xiaomi/MIUI, Samsung и др. путь может слегка отличаться — ищите тот же ⋮ в «О приложении». На Android 12 и старше ограничения обычно нет — включайте сразу.\n\nКлавиши громкости только читаются, никогда не блокируются."
    override val volOpenAppInfo = "Открыть «О приложении»"
    override val volAccessOn = "Спец.возможности: включены"
    override val volAccessOff = "Спец.возможности: выключены"
    override val volOpenAccess = "Открыть настройки спец.возможностей"
    override val volGapLabel = "Макс. мс между нажатиями"
    override val volPatternsTitle = "Распознаваемые паттерны"
    override val volPatternPick = "Паттерн"
    override val volPatternsLegend = "↑ = громкость вверх, ↓ = вниз"
    override val volNoRights = "Приложение пока НЕ имеет прав на обработку кнопок громкости — выдайте доступ по инструкции внизу страницы."
    override val volGrantShort = "Доступа к спец.возможностям пока нет. Прочитайте подробную инструкцию внизу этой страницы и нажмите «Проверить»."
    override val volCheck = "Проверить"
